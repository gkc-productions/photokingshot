  ...
  if (!mounted) {
    return (
      <div className="inline-flex rounded-sm border border-[var(--border)] bg-[var(--card)] p-1" aria-label="Theme mode loading">
        <span className="min-h-8 rounded-sm bg-[var(--gold)] px-3 py-2 text-xs font-black uppercase tracking-wide text-black">Auto</span>
      </div>
    );
  }
      ...
      {options.map(([value, label]) => (
        <button
          ...
          className={`min-h-8 rounded-sm px-3 text-xs font-black uppercase tracking-wide transition ${
            (theme || "system") === value ? "bg-[var(--gold)] text-black" : "text-[var(--muted)] hover:text-[var(--foreground)]"
          }`}
        >
          {label}
        </button>
