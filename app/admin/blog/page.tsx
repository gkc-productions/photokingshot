  ...
  return (
    <section className="section-shell py-10">
      <div className="flex items-center justify-between gap-4">
        ...
        <Link href="/admin/blog/new" className="rounded-sm bg-[#d6a83f] px-4 py-3 text-sm font-black text-black">New Post</Link>
      </div>
      {!result.hasDb ? <div className="mt-6"><DbNotice area="blog admin" /></div> : null}
      <div className="mt-8 grid gap-4">
        {result.posts.map((post) => (
          <article key={post.id} className="rounded-sm border border-white/10 bg-white/[0.04] p-5">
            <p className="eyebrow">{post.status} / {post.category}</p>
            <h2 className="mt-2 text-2xl font-black">{post.title}</h2>
            <p className="mt-2 text-white/64">{post.excerpt}</p>
            <div className="mt-4 flex gap-3">
              <Link href={`/admin/blog/${post.id}/edit`} className="text-[#d6a83f]">Edit</Link>
